...
// List lists all CloudProviders in the indexer.
func (s *cloudProviderLister) List(selector labels.Selector) (ret []*v1alpha1.CloudProvider, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.CloudProvider))
	})
	return ret, err
}

// Get retrieves the CloudProvider from the index for a given name.
func (s *cloudProviderLister) Get(name string) (*v1alpha1.CloudProvider, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("cloudprovider"), name)
	}
	return obj.(*v1alpha1.CloudProvider), nil
}
