...
type ClusterCredentialLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ClusterCredential, err error)
	// Get retrieves the ClusterCredential from the index for a given name.
	Get(name string) (*v1alpha1.ClusterCredential, error)
...
// List lists all ClusterCredentials in the indexer.
func (s *clusterCredentialLister) List(selector labels.Selector) (ret []*v1alpha1.ClusterCredential, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ClusterCredential))
	})
	return ret, err
}

// Get retrieves the ClusterCredential from the index for a given name.
func (s *clusterCredentialLister) Get(name string) (*v1alpha1.ClusterCredential, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("clustercredential"), name)
	}
	return obj.(*v1alpha1.ClusterCredential), nil
}
